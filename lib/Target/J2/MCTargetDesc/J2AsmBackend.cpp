...
void J2AsmBackend::applyFixup(const MCAssembler &Asm, const MCFixup &Fixup,
  ...
  case J2::fixup_J2_PC2_8:
    applyDisplacement<8 /* Size */, 2 /* Multiply */>(Value,
                                                      Data.slice(Offset));
    break;
  case J2::fixup_J2_PC4_8: {
    applyDisplacement<8 /* Size */, 4 /* Multiply */>(Value,
                                                      Data.slice(Offset));
    break;
  }
  case FK_Data_4:
    *reinterpret_cast<uint32_t *>(Data.slice(Offset).data()) = Value / 4;
    break;
  default:
    llvm_unreachable("Unkown fixup!");
  }
...
void J2AsmBackend::adjustFixupValue(const MCFixup &Fixup, const MCValue &Target,
